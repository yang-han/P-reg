...
def loss_(model, data, mask, beta):
    ...
    nll_loss = F.nll_loss(torch.log_softmax(logit[mask], dim=1), data.y[mask])
    reg_loss = laplacian(data.edge_index, logit)
    loss = nll_loss + beta*reg_loss
    return loss
...
def run():
    ...
    if args.verbose:
        ...
        log = "Dataset: {}, Epoch: {:03d}, Train: {:.4f}, Val: {:.4f}, Test: {:.4f}.(final layer)"
        print(log.format(dataset, int(data_avr[3]), data_avr[0], data_avr[1], data_avr[2]))
        para = str(args.beta)+'_'+str(lr)+'_'+str(weight_decay)+'_'+str(patience)+'_'+str(hidden_size)
        outfile = args.dataset+'_'+para+'.npy'
        with open(os.path.join(path, "result", args.model.lower()+'_edge', outfile), 'wb') as f:
            np.save(f, result)
