...
class PREGGNN_node(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        x = self.m1(batched_data)
        return self.conv(x, edge_index, edge_attr)

class PREGGNN_node_Virtualnode(torch.nn.Module):
    def __init__(self, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_type = 'gin'):
        super(PREGGNN_node_Virtualnode, self).__init__()
        self.m1 = GNN_node_Virtualnode(num_layer, emb_dim, drop_ratio, JK, residual, gnn_type)
        self.conv = IConv(emb_dim)
        # self.mu = nn.Parameter(torch.ones(1)* 0.2, requires_grad=False)
